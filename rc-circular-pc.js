function CircularPC(options){
    var _cpc=this,
        _cpcID=(Math.floor(Math.random()*10000)+'').substr(0,4),
        _cpcCanvas=null,
        defaults={"size":300,"barColor1":"#000","barColor2":"#d12","textColor":"#000","barSize":15,"bgColor":"none"};
    ...
    function doAnim(cpc,fpc,stp,spd){
        if(cpc>fpc) cpc=fpc;
        _cpc.value(cpc);
        if(cpc<fpc) setTimeout(function(){doAnim(cpc+stp,fpc,stp,spd)},spd);
    }
    ...
    function drawMe(pc){
        var p=pc/100,
            ...
            color1=new Color(_cpc.barColor1),
            color2=new Color(_cpc.barColor2),
            barColor=color1.shiftTo(color2,p).cssRGB();
        c.clearRect(0,0,d,d);
...
window.onload=function(){
    new CircularPC({barSize:30,size:200}).appendTo("pc-main").animate({value:75});
    new CircularPC({barSize:30,size:200}).appendTo("pc-main").animate({value:55});
    new CircularPC({barSize:30,size:200}).appendTo("pc-main").animate({value:15});
}
